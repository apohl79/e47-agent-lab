...
| `project-context-curator` | `0.3.0` | Durable scoped context with optional cross-project hybrid retrieval. | `plugins/project-context-curator` | Codex, Claude Code |
...
mode, so it does not install or run a Qdrant server. After approval, provision
the pinned Python packages and models with
`./install.sh --with-context-runtime`.
...
start because those update paths do not execute this installer.
