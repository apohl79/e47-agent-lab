...
- If `HARNESS` is `codex`, resolve `CODEX_ACTIVE_PROVIDER` and `CODEX_ACTIVE_MODEL` from the SessionStart hook context. `CLAUDE_PROFILE` is only a fallback profile capability for launching a Claude CLI reviewer; it does NOT mean the current Codex orchestrator is using Claude/Anthropic.
  - If `CODEX_ACTIVE_PROVIDER` is `anthropic` or `claude`, use the **Codex-host + Claude/Anthropic-provider reviewer set** — the orchestrator's active provider is Claude/Anthropic, so the required same-provider (Claude-lens) reviewers run through builtin `spawn_agent`, which inherits that active provider. The opposite-provider (GPT-lens) secondary uses `spawn_agent` with `agent_type: "general-purpose-gpt"` when that agent definition is available; otherwise fall back to the CLI dispatch in "Opposite-provider secondary dispatch".
  - If `CODEX_ACTIVE_PROVIDER` is `openai`, use the **Codex-host + OpenAI-provider reviewer set** — the orchestrator's active provider is OpenAI, so the required same-provider (GPT-lens) reviewers run through builtin `spawn_agent`, which inherits that active provider. The opposite-provider (Claude-lens) secondary uses `spawn_agent` with `agent_type: "general-purpose-claude"` when that agent definition is available; otherwise fall back to the CLI dispatch in "Opposite-provider secondary dispatch".
...
When the orchestrator runs in Codex, there are four dispatch mechanisms, chosen by reviewer role, agent-definition availability, and dispatch outcome:

- **Same-provider reviewers use builtin `spawn_agent` without `agent_type`** (Primary, Security, and any selected Architecture / Observability / Integration). They inherit the orchestrator's active provider.
- **Typed external-provider reviewers prefer builtin `spawn_agent` with a general-purpose agent definition.** The opposite-provider secondary uses `general-purpose-gpt` or `general-purpose-claude`; Gemini uses `general-purpose-gemini`. Do not pass a `provider` argument; `agent_type` selects the preconfigured provider.
- **Definition-unavailable typed reviewers use their existing CLI fallback.** The secondary uses the `codex` CLI; Gemini uses the configured Gemini CLI.
- **A failed typed Claude secondary gets one direct Claude CLI retry.** When `CODEX_ACTIVE_PROVIDER=openai`, a failed or empty `general-purpose-claude` result is retried once with the `claude` CLI when that binary is available.
- **A completed-empty or invalid typed Gemini result gets one corrective same-agent follow-up.** Send `followup_task` to the existing `gemini_review` agent with the correction defined under "Typed Gemini terminal-result validation". This resumes the same reviewer context; do not switch to the Gemini CLI while the typed agent definition is available.

Resolve the active first-party provider from `CODEX_ACTIVE_PROVIDER`, not from profile existence:

- `CODEX_ACTIVE_PROVIDER=anthropic` or `claude` -> active provider is Claude/Anthropic; required Claude-lens reviewers use builtin `spawn_agent` inheriting that provider; the opposite-provider secondary is the OpenAI-backed `general-purpose-gpt` agent, dispatched with `spawn_agent(agent_type: "general-purpose-gpt", reasoning_effort: "<reviewer_reasoning_effort>")` when available.
- `CODEX_ACTIVE_PROVIDER=openai` -> active provider is OpenAI; required GPT-lens reviewers use builtin `spawn_agent` inheriting OpenAI; the opposite-provider secondary is the Claude-backed `general-purpose-claude` agent, dispatched with `spawn_agent(agent_type: "general-purpose-claude", reasoning_effort: "<reviewer_reasoning_effort>")` when available.
- `CLAUDE_PROFILE=<name>` may be present in either case. Use it only as the CLI fallback profile for a Claude secondary when `CODEX_ACTIVE_PROVIDER=openai` and `general-purpose-claude` is unavailable.

**Reviewer reasoning effort.** Resolve the current Codex session's reasoning effort from active session metadata, never from shell environment or model name. Compute `reviewer_reasoning_effort` as `low -> low`, `medium -> medium`, `high -> high`, and `xhigh|max|ultra -> xhigh`. Pass `reasoning_effort: "<reviewer_reasoning_effort>"` explicitly to EVERY reviewer `spawn_agent` call: same-provider reviewers, the typed opposite-provider secondary, and typed Gemini. CLI fallbacks and Claude-host Agent calls are unchanged.
...
**Opposite-provider secondary dispatch.** First inspect the current `spawn_agent` tool metadata / available agent-type list:
- active provider `claude` / `anthropic` (secondary = OpenAI): if `general-purpose-gpt` is listed, launch `spawn_agent` with `agent_type: "general-purpose-gpt"`, `fork_turns: "none"`, `reasoning_effort: "<reviewer_reasoning_effort>"`, `task_name: "secondary_review"`, and the secondary prompt as the `message`.
- active provider `openai` (secondary = Claude): if `general-purpose-claude` is listed, launch `spawn_agent` with `agent_type: "general-purpose-claude"`, `fork_turns: "none"`, `reasoning_effort: "<reviewer_reasoning_effort>"`, `task_name: "secondary_review"`, and the secondary prompt as the `message`.

If the required `general-purpose-*` agent definition is not listed, fall back to the local `codex` CLI with explicit provider/profile selection, reading the secondary prompt from stdin:
- active provider `claude` / `anthropic` (secondary = OpenAI): the OpenAI provider is the base config default, so run `codex` with NO `-p` profile — `timeout <reviewer_timeout_s> codex exec --dangerously-bypass-approvals-and-sandbox - < <abs-secondary-prompt-file>` (do not layer any profile). Confirm the base config resolves to the OpenAI provider/model that backs `general-purpose-gpt`; if the base default is not OpenAI, select it explicitly with `-c model_provider=openai -c model=<gpt-model>`.
- active provider `openai` (secondary = Claude): if a Claude profile name is resolved from the SessionStart hook context, layer that profile with the global `-p` flag placed BEFORE the `exec` subcommand — `timeout <reviewer_timeout_s> codex -p <CLAUDE_PROFILE> exec --dangerously-bypass-approvals-and-sandbox - < <abs-secondary-prompt-file>`. If no Claude profile is resolved, the CLI fallback is unavailable; skip the secondary if `general-purpose-claude` is also unavailable.
If active provider is `openai`, `general-purpose-claude` was dispatched, and that dispatch fails (including a tool error such as HTTP 429) or returns no output, retry the same prompt exactly once through the direct Claude CLI when `command -v claude` succeeds: `timeout <reviewer_timeout_s> claude --dangerously-skip-permissions -p "$(cat <abs-secondary-prompt-file>)" </dev/null`. This retry does not require `CLAUDE_PROFILE`; the Claude CLI owns its provider configuration. Do not retry after a successful typed result, and do not apply this failure retry to `general-purpose-gpt`.
...
If neither the corresponding `general-purpose-*` agent definition nor the CLI fallback profile/base-provider can be resolved, skip the secondary reviewer and record `"secondary reviewer: skipped (opposite-provider <agent-type> unavailable)"` in `notes`. Do not fall back to a same-provider `spawn_agent` for the secondary (that defeats the two-provider lens) and do not block on it. Builtin `spawn_agent` is assumed available in Codex mode for the required same-provider reviewers; if the builtin spawn mechanism itself is unavailable, block.

**Gemini dispatch.** Inspect the current `spawn_agent` agent-type list. If `general-purpose-gemini` is listed, launch Gemini with `agent_type: "general-purpose-gemini"`, `fork_turns: "none"`, `reasoning_effort: "<reviewer_reasoning_effort>"`, `task_name: "gemini_review"`, and the Gemini prompt as an explicit imperative `message`. If it is not listed, use the configured Gemini CLI fallback in "Gemini delivery". After a typed Gemini agent completes, validate its terminal result and apply the one allowed same-agent corrective follow-up when needed.

1. **Codex Primary** — focused Codex builtin `spawn_agent` sub-agent inheriting `CODEX_ACTIVE_PROVIDER` (`anthropic`/`claude` or `openai`) — **required**.
2. **Secondary first-party** — opposite-provider reviewer dispatched via `spawn_agent(agent_type: "general-purpose-gpt", reasoning_effort: "<reviewer_reasoning_effort>")` when active provider is `claude` / `anthropic`, or `spawn_agent(agent_type: "general-purpose-claude", reasoning_effort: "<reviewer_reasoning_effort>")` when active provider is `openai`; fall back to the `codex` CLI if the required agent definition is unavailable. A failed or empty typed Claude result gets one direct Claude CLI retry when available — **optional / can-fail**. Correctness + data-flow lens. Skip if neither the typed agent nor its applicable CLI path produces output.
3. **Gemini** — typed `spawn_agent(agent_type: "general-purpose-gemini", reasoning_effort: "<reviewer_reasoning_effort>")` when available; otherwise the configured Gemini CLI fallback (see "Gemini delivery") — **can-fail**.
4. **Codex Security** — focused Codex builtin `spawn_agent` sub-agent using the active provider — **required**. Skill per "Security reviewer skill selection".
5. **Codex Architecture** — focused Codex builtin `spawn_agent` sub-agent using the active provider — **conditional**. **Required-when-it-runs**. Loads the resolved architecture recipes by reading their on-disk `SKILL.md` path(s) (Codex sub-agents have no `Skill` tool — see "Host skill-loading mechanism").
6. **Codex Observability** — focused Codex builtin `spawn_agent` sub-agent using the active provider — **conditional**. **Required-when-it-runs**. Loads the observability recipes by reading their resolved on-disk `SKILL.md` path(s) (Codex sub-agents have no `Skill` tool — see "Host skill-loading mechanism").
7. **Codex Integration** — focused Codex builtin `spawn_agent` sub-agent using the active provider — **conditional**. **Required-when-it-runs**. Loads no recipe.

Architecture, Observability, and Integration are decided independently — a run may include any combination, or none.

The required base slots, the secondary reviewer when selected, and any selected conditional reviewers must all be launched in the same initial parallel batch. Do not launch initial reviewers sequentially. The only allowed follow-on actions are the direct Claude CLI retry after a failed or empty `general-purpose-claude` result and the one corrective `followup_task` to an existing typed Gemini reviewer. Do not substitute a CLI reviewer for a required Codex host-native reviewer.

CLI invocation parameters are fixed: `codex --dangerously-bypass-approvals-and-sandbox exec` for Codex when `HARNESS == claude`; `claude --dangerously-skip-permissions -p "<prompt>" </dev/null` for the failed typed-Claude secondary retry; `gemini -y --skip-trust -p "<prompt>" </dev/null` for Gemini on Claude hosts and as the Codex fallback (the whole reviewer prompt goes in `-p`; stdin is redirected from `/dev/null` so the CLI does not block waiting on stdin). **JSON stream output is disabled** — do NOT pass `--json` to `codex`, `--output-format stream-json` to `claude`, or `-o stream-json` / `-o json` to `gemini` (use default text output). Plain stdout is what this skill consumes.
...
- **Codex opposite-provider secondary (optional):** if the required `general-purpose-*` agent definition is available, dispatch it with `spawn_agent`. When active provider is `openai` and the `general-purpose-claude` dispatch fails or returns no output, retry once through the direct Claude CLI if installed; apply the external CLI failure semantics to that retry. A failed `general-purpose-gpt` dispatch, or a failed typed-Claude dispatch with no available/successful Claude CLI retry, is recorded unavailable with zero findings. If the agent definition is unavailable but the existing `codex` CLI fallback can be resolved, dispatch that fallback. A non-zero CLI exit with non-empty stdout is a normal review.
- **Codex Gemini (can-fail):** if `general-purpose-gemini` is available, dispatch it with `spawn_agent`. If dispatch itself fails, record Gemini unavailable. If the agent completes with empty or invalid output, send one corrective `followup_task` to that same agent and collect its next terminal result without a lifetime deadline. Record Gemini unavailable with zero findings only when the same-agent follow-up also returns empty/invalid output or fails. If the definition is unavailable, use the CLI fallback and apply the CLI failure semantics below; do not use CLI as recovery for an available typed agent.
...
**Typed Gemini terminal-result validation.** After trimming whitespace, accept only exact `STATUS: OK` or one or more complete finding blocks (optionally wrapped in Markdown fences) with all seven required fields: `CATEGORY`, `SEVERITY`, `ORIGIN`, `WHERE`, `ISSUE`, `WHY`, and `FIX`. Treat every other typed response as invalid, including a greeting, confirmation request, question, plan, progress-only message, refusal, lifecycle instruction, or unrelated prose. Preserve the exact invalid output in `notes` when it is at most 1,000 characters; otherwise preserve its first 1,000 characters and total character count so the failure remains diagnosable without flooding the report.

Before recording a typed Gemini reviewer unavailable for empty or invalid output, send at most one `followup_task` to the existing `gemini_review` agent with this message verbatim: `Your previous response did not return a review result. Continue the original assigned review now on the same agent. Execute it without asking for confirmation or waiting for input. Use read-only tools and do not edit files. Return only one or more complete finding blocks in the required format, or exactly STATUS: OK.` Do not include the invalid output in the follow-up message. Let the resumed native agent run to natural completion, then validate the new terminal result by the same rule. If it is still empty or invalid, record Gemini unavailable with both outputs and continue with zero Gemini findings. Never spawn a replacement Gemini agent or invoke the Gemini CLI for this recovery.
...
1. Resolve `attempt` (default `1`). Resolve `HARNESS`; block unless `claude` or `codex`. Omitted review-context inputs are not blockers. When `HARNESS == codex`, resolve `CODEX_ACTIVE_PROVIDER`, `CODEX_ACTIVE_MODEL`, and `reviewer_reasoning_effort` from current session context, capping the latter at `xhigh`. If active provider is `anthropic`/`claude`, required same-provider reviewers use builtin `spawn_agent` inheriting that provider; if active provider is `openai`, required same-provider reviewers use builtin `spawn_agent` inheriting OpenAI. Treat `CLAUDE_PROFILE`, when present, only as a `codex` CLI fallback profile for a definition-unavailable Claude secondary. Resolve the opposite-provider secondary's typed-agent dispatch first (`general-purpose-gpt` for Claude/Anthropic active provider, `general-purpose-claude` for OpenAI active provider), then its `codex` CLI fallback only if the definition is unavailable; for a failed or empty typed Claude result, resolve direct Claude CLI availability for the one-time retry. Resolve Gemini the same way: `general-purpose-gemini` first, then the configured Gemini CLI fallback only if that definition is unavailable.
...
12. Launch all selected reviewers in a single parallel batch (all tool calls in one assistant message — do NOT chain them):
    ...
    When `HARNESS == codex`:
    - **Primary / Security / Architecture / Observability / Integration** — use builtin `spawn_agent` with `fork_turns: "none"`, `reasoning_effort: "<reviewer_reasoning_effort>"`, and role-specific `task_name` (`primary_review`, `security_review`, `architecture_review`, `observability_review`, `integration_review`). These inherit `CODEX_ACTIVE_PROVIDER` (`anthropic`/`claude` or `openai`) — `spawn_agent` has no `provider` argument, so do NOT attempt to pass one. Pass each prompt verbatim. Codex sub-agents have NO `Skill` tool, so any skill a reviewer must use is delivered as a resolved absolute `SKILL.md` path in its prompt and the reviewer reads that file (see "Host skill-loading mechanism" / "Codex skill-path resolution") — do NOT instruct a Codex sub-agent to invoke a `Skill` tool. Because a fresh sub-agent may otherwise return a bare greeting, the `message` MUST be an explicit imperative instruction to perform the review now (read the prompt file / read the resolved recipe `SKILL.md` path(s) / run `git diff` / emit findings), not just a pointer to context.
    - **Secondary first-party (opposite-provider)** — prefer `spawn_agent` with `fork_turns: "none"`, `reasoning_effort: "<reviewer_reasoning_effort>"`, `task_name: "secondary_review"`, and an explicit imperative `message`. Active provider `claude`/`anthropic` -> `agent_type: "general-purpose-gpt"`; active provider `openai` -> `agent_type: "general-purpose-claude"`. Launch it in the same initial parallel batch as the other reviewers. If the required `general-purpose-*` agent definition is unavailable, fall back to a Bash CLI subprocess: active provider `claude`/`anthropic` -> `timeout <reviewer_timeout_s> codex exec --dangerously-bypass-approvals-and-sandbox - < <abs-secondary-prompt-file>` (base config = OpenAI/`general-purpose-gpt`, no `-p`); active provider `openai` with a resolved Claude profile -> `timeout <reviewer_timeout_s> codex -p <CLAUDE_PROFILE> exec --dangerously-bypass-approvals-and-sandbox - < <abs-secondary-prompt-file>` (Claude/`general-purpose-claude`; the global `-p` goes BEFORE `exec`). No `--json`. If the `general-purpose-claude` dispatch was attempted but failed (including HTTP 429) or returned no output, and `command -v claude` succeeds, apply the curator CLI gate and retry once with `timeout <reviewer_timeout_s> claude --dangerously-skip-permissions -p "$(cat <abs-secondary-prompt-file>)" </dev/null`; do not pass `--output-format stream-json`. If neither applicable path produces output, record the secondary unavailable. The `-` on `codex exec` reads the prompt from stdin; the Claude CLI receives the same prompt through `-p` and closed stdin.
    - **Gemini** — if `general-purpose-gemini` is available, use `spawn_agent` with `agent_type: "general-purpose-gemini"`, `fork_turns: "none"`, `reasoning_effort: "<reviewer_reasoning_effort>"`, `task_name: "gemini_review"`, and an explicit imperative `message`; launch it in the same parallel batch. Otherwise use the same Gemini Bash command as above with `<reviewer_timeout_s>`. After a typed agent completes, validate its terminal result; for completed-empty or invalid output, send one corrective `followup_task` to that same agent before recording it unavailable.
...
1. **Parse** every finding block from every collected output and the verification-run findings. Output not following the format is still triaged — extract the substance — except a typed Gemini terminal response that failed the explicit validation above; follow up or mark that reviewer unavailable before triage.
...
- `reviewer_set` — reviewers used (three to seven), including the detected `HARNESS`, Codex active provider/model (`CODEX_ACTIVE_PROVIDER` and `CODEX_ACTIVE_MODEL` when `HARNESS == codex`), and each reviewer's concrete implementation (`Claude Agent tool`, `spawn_agent` inheriting active provider `anthropic`|`openai`, opposite-provider `spawn_agent` secondary `general-purpose-claude`|`general-purpose-gpt`, opposite-provider `codex` CLI secondary fallback `general-purpose-claude`|`general-purpose-gpt`, direct Claude CLI secondary retry after failed `general-purpose-claude`, Gemini `spawn_agent` `general-purpose-gemini`, same-agent Gemini `followup_task`, or Gemini CLI fallback). For Security record the resolved security skill load list (or `lite` when the fallback was used); for Architecture the architecture load list; for Observability the observability load list; for Integration the access mode + external repos. Mark any can-fail reviewer recorded unavailable and any optional secondary skipped.
