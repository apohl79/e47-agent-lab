...
// Small CLI bundle used by the discuss skill to discover running Codex
// sessions through the generic app-server protocol.
await build({
  entryPoints: [join(__dirname, 'src/session-control.ts')],
  outfile: join(distDir, 'session-control.js'),
  bundle: true,
  platform: 'node',
  target: 'node20',
  format: 'esm',
