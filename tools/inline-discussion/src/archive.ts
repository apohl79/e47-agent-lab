...
export function parseArchivedThreads(markdown: string, documentPath?: string): Thread[] {
  return findArchivedBlocks(markdown).map((b) => documentPath ? { ...b.thread, documentPath } : b.thread);
}
...
export function findArchivedBlocks(markdown: string): ArchivedBlockInfo[] {
  ...
  for (let i = 0; i < blocks.length; i += 1) {
    ...
    const occurrenceMatch = block.markdown.match(/^<details[^>]*\bdata-occurrence="(\d+)"[^>]*>/i);
    const occurrence = occurrenceMatch ? Number.parseInt(occurrenceMatch[1]!, 10) : 1;
    const anchor: Anchor = quote
      ? { blockId: anchorBlock.id, quote, occurrence: occurrence > 0 ? occurrence : 1 }
