...
function formatDetails(input: AppendThreadInput): string {
  ...
  const occurrenceAttr = input.quote && Number.isInteger(input.occurrence) && input.occurrence! > 0
    ? ` data-occurrence="${input.occurrence}"`
    : '';
  const detailsAttrs = `${idAttr}${occurrenceAttr}`;
  const lines: string[] = [];

  if (input.kind === 'note') {
    lines.push(`<details${detailsAttrs}><summary>📝 Note on ${label} — ${date}</summary>`);
    lines.push(
  ...
  lines.push(`<details${detailsAttrs}><summary>💬 Thread on ${label} — ${date}</summary>`);
