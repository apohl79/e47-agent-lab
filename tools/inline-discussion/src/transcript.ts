...
export function readJsonl(path: string | undefined | null): JsonlEntry[] {
  // Standalone CLI use (`inline-discussion <doc>`) has no host session, so
  // the launcher omits the JSONL path. Treat that — and a path that simply
  // does not exist on disk — as "empty transcript".
